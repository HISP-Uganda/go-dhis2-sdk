/*
DHIS2 API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.40
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package models

import (
	"encoding/json"
)

// checks if the AttributeValue type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AttributeValue{}

// AttributeValue struct for AttributeValue
type AttributeValue struct {
	Attribute *AttributeValueAttribute `json:"attribute,omitempty"`
	Value     *string                  `json:"value,omitempty"`
}

// NewAttributeValue instantiates a new AttributeValue object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAttributeValue() *AttributeValue {
	this := AttributeValue{}
	return &this
}

// NewAttributeValueWithDefaults instantiates a new AttributeValue object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAttributeValueWithDefaults() *AttributeValue {
	this := AttributeValue{}
	return &this
}

// GetAttribute returns the Attribute field value if set, zero value otherwise.
func (o *AttributeValue) GetAttribute() AttributeValueAttribute {
	if o == nil || IsNil(o.Attribute) {
		var ret AttributeValueAttribute
		return ret
	}
	return *o.Attribute
}

// GetAttributeOk returns a tuple with the Attribute field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AttributeValue) GetAttributeOk() (*AttributeValueAttribute, bool) {
	if o == nil || IsNil(o.Attribute) {
		return nil, false
	}
	return o.Attribute, true
}

// HasAttribute returns a boolean if a field has been set.
func (o *AttributeValue) HasAttribute() bool {
	if o != nil && !IsNil(o.Attribute) {
		return true
	}

	return false
}

// SetAttribute gets a reference to the given AttributeValueAttribute and assigns it to the Attribute field.
func (o *AttributeValue) SetAttribute(v AttributeValueAttribute) {
	o.Attribute = &v
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *AttributeValue) GetValue() string {
	if o == nil || IsNil(o.Value) {
		var ret string
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AttributeValue) GetValueOk() (*string, bool) {
	if o == nil || IsNil(o.Value) {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *AttributeValue) HasValue() bool {
	if o != nil && !IsNil(o.Value) {
		return true
	}

	return false
}

// SetValue gets a reference to the given string and assigns it to the Value field.
func (o *AttributeValue) SetValue(v string) {
	o.Value = &v
}

func (o AttributeValue) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AttributeValue) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Attribute) {
		toSerialize["attribute"] = o.Attribute
	}
	if !IsNil(o.Value) {
		toSerialize["value"] = o.Value
	}
	return toSerialize, nil
}

type NullableAttributeValue struct {
	value *AttributeValue
	isSet bool
}

func (v NullableAttributeValue) Get() *AttributeValue {
	return v.value
}

func (v *NullableAttributeValue) Set(val *AttributeValue) {
	v.value = val
	v.isSet = true
}

func (v NullableAttributeValue) IsSet() bool {
	return v.isSet
}

func (v *NullableAttributeValue) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAttributeValue(val *AttributeValue) *NullableAttributeValue {
	return &NullableAttributeValue{value: val, isSet: true}
}

func (v NullableAttributeValue) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAttributeValue) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
